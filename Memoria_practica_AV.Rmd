...
#Tablas
A continuación procedo a presentar las tablas que vamos a utilizar a lo largo de nuestro estudio. Se presentan aquí, dada su aparición en múltiples apartados, facilitándose así su ejecución:
...
Antes de continuar con el estudio, reiniciamos nuestras tablas originales:
```{r}
nadadoresParticipantes<-nadadoresParticipantesCopia
nadadoresPruebas<-nadadoresPruebasCopia
#Vemos que desaparece iso,continentes...
nadadoresPruebas
```


## Estudio sobre los eventos, reactiontime, lane, heats, daytime: [Alonso. Sólo el inicio.]
...
```{r, echo=FALSE}
rm(limite_inferior, limite_inferior1, limite_superior, limite_superior1, tiempos_hhmm, convertir_a_hhmm)
```
...
```
Podemos replantearnos ahora si esto se mantiene indistintamente del género del nadador.Es decir,¿En las pruebas femeninas de 200 m, también se nadan todos los estilos?
