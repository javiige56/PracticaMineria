...
Bien, visto que el mejor es k=3, vamos a continuación a evaluar mi modelo con k=3 y escalando. A continuación entrenamos el modelo y vemos la matriz de confusión en los datos de entrenamiento: 
...
Parece que se produce una mejora respecto al anterior modelo tanto en accuracy como en sensitivity. A continuación, evalúo mi modelo en el conjunto de test. Para ello, primero escalo mis datos de test:

```{r}
df_test_scaled <- predict(preProcValues, df_test %>% select(-target))  # Escalar todas menos el target
```

Y ahora, hago las predicciones: 
...
El modelo tiene un buen rendimiento en el conjunto de entrenamiento con una sensibilidad alta (85.7%). Esto indica que el modelo está capturando patrones en los datos de entrenamiento. Pero, en el test, tanto el accuracy como la sensibilidad caen, 53.06% (accuracy), 72% (sensitivity), lo que sugiere un sobreajuste a mis datos de entrenamiento.
...
El modelo tiene un buen desempeño en el conjunto de entrenamiento, con una sensibilidad razonable (81.25%). Y accuracy(76.41%)

El modelo en el conjunto de test es mejor en este caso (sensitivity del 72%), y un accuracy de 67.35%, lo que sugiere una mejora significativa respecto a versiones previas.
