...
rm(df,df_test,dist_train,dist_test,matriz_confusion, x_test)
```
...
rm(df_arbol, df_test_arbol, pred_bag_test, pred_bag_train, clase.pred.bag.test, clase.pred.bag.train)
```
...
clase_pred_rf_train <- ifelse(pred_rf_train > 0.66, "1", "0")  
...
clase_pred_rf_test <- ifelse(pred_rf_test > 0.66, "1","0")   
...
El umbral del 0.66 puede no ser óptimo. Calculamos el mejor umbral (best_threshold) de decisión para el modelo basado en la curva ROC.

```{r}
roc_curve <- roc(df_test_random$target, pred_rf_test)
best_threshold <- coords(roc_curve, "best", ret = "threshold")
print(best_threshold)

```
El umbral óptimo según este criterio, es 0.7. Por tanto, realizamos las predicciones de nuestros datos con este corte. 
...
Parece que hemos maximizado la especificidad gracias a este cambio. Sin embargo, existe bastante error respecto a la predicción de los competidores que no bajan de marca. Podríamos realizar un subgrupo más, basado en su aleatoriedad. Es decir, no tenemos conclusiones claras de que bajen o no de marca en su final. 
...
pred_rf_test3 <- predict(rf_model, df_test_random, type = "prob")[, 2]
...
predictor_rf2 <- Predictor$new(
  model = rf2,
  data = df_random,  # Variables explicativas
...
roc_2<- roc(df_test2$target, probs_knn2)
roc_3<- roc(df_test_random$target, probs_random)
roc_4<- roc(df_test_arbol$target, probs_bag2)
...
```{r}
df_final <- datos2015Target_validation %>% select(target, distance, stroke, gender, name,heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)

df_final$distance<- as.factor(df_final$distance)

pred_bag <- predict(modeloBagging2, df_final ,type="class")

cf_bag_val <- caret::confusionMatrix(pred_bag, as.factor(df_final$target),positive="1")
cf_bag_val
```
