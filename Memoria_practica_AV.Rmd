...
```{r}
((0.7166871-0.6922862)/(0.692286))*100
```

Obtenemos que las mujeres tardan un 3.5% más de tiempo que los hombres. A efectos prácticos, tardar 2 centésimas más en reaccionar no es significativo en pruebas de fondo, pero sí que lo puede llegar a ser en pruebas de velocidad donde la diferencia entre los nadadores es de décimas. 
...
ggplot(nadadoresPruebas, aes(x = reactiontime, color = as.factor(distance), fill = as.factor(distance))) +
  geom_density(alpha = 0.8) +  # Ajustar la transparencia
...
Luego, observamos que estamos en lo cierto.
Realizamos la partición de los datos sobre el dataframe creado anteriormente:
...
#Tras dividir los datos, tomamos los de train y volvemos a dividir.
...
```{r}
# Eliminar las tablas que no se usarán

rm(datos2015, nadadoresParticipantes, nadadoresPruebas, nadadoresParticipantesCopia, nadadoresPruebasCopia, nadadoresFinalistas, indices_entrenamiento, indices_test, indices_train, indices_validation )
rownames(datos2015Target_train)<-NULL
rownames(datos2015Target_test)<-NULL
rownames(datos2015Target_validation)<-NULL
```
...
Tenemos que un 57.43% baja de marca.
Tiene sentido intentar buscar mediante medidas de rendimiento, esas predicciones sobre nuevas observaciones.
Vamos a evaluar todos los modelos de rendimiento vistos en teoría, y elegiremos el(los) que nos aporten un mayor rendimiento a través del análisis de sus medidas.

## Variables que pueden influir en la respuesta.

Evaluamos en nuestro conjunto de datos si existe algún patrón que nos pueda ayudar a predecir si las nadadoras bajan o no de tiempo. Analizamos si existen variables que dictaminan más que otras.

Analizamos en primer lugar la edad. Evaluamos si existe algún tipo de relación entre la edad y si bajan o no tiempo.
Para ello,
...
porcentajes <- df %>%
  dplyr::group_by(grupo_edad, target) %>%
  dplyr::summarise(total = n(), .groups = "drop") %>%
  dplyr::group_by(grupo_edad) %>%
  dplyr::mutate(porcentaje = total / sum(total) * 100)
...
A partir de los 30 años, aumenta el porcentaje de nadadores que no mejoran su marca, lo que sugiere una posible relación que será considerada.

```{r, echo=FALSE}
rm(conteo, df, porcentajes)
```

Continuamos nuestro estudio con la variable stroke, analizando si alguno de los 5 estilos tiene relación con la mejora de la marca.
...
Observamos que algunos estilos son más influyentes que otros. Medley y Free muestran un porcentaje elevado de mejora, mientras que en el estilo Braza no parece ser frecuente bajar la marca en las finales.

A continuación, analizamos si existe una relación entre las distancias y la mejora de la marca. El siguiente gráfico ilustra este análisis:
...
# Calcular el porcentaje de nadadores que bajan (1) y que no bajan (0) por Stroke
porcentajes <- datos %>%
  dplyr::group_by(distance, target) %>%
  dplyr::summarise(total = n(), .groups = "drop") %>%
...
En los modelos que calcularemos a continuación, nuestro objetivo será garantizar que, cada vez que afirmemos que un nadador mejora su tiempo, esta predicción sea correcta. Por lo tanto, nos centraremos en minimizar la tasa de falsos positivos.
...
```{r, echo=FALSE}
tabla <- data.frame(
  "Prediction 0" = c("TN", "FP"),  
  "Prediction 1" = c("FN", "TP")   
)

rownames(tabla) <- c("Reference 0", "Reference 1")

knitr::kable(tabla, format = "markdown", align = "c", caption = "Matriz de Confusión")
...
-   FPR (tasa de falsos positivos) $= \frac{FP}{FP+TP}$ Intentaremos minimizar este valor, ya que mide la proporción de positivos que acierta el modelo. Esta medida de rendimiento, no se contempla en las matrices de confusión y debemos calcularla a mano. 

De manera análoga al FPR, podemos usar $= \frac{TP}{FP+TP}$ y tratar de maximizar este valor. Este valor sí que se encuentra en las matrices de confusión que evaluaremos más adelante. 
...
matrizconfusion1 <- caret::confusionMatrix(predicciones, as.factor(df$target), positive = "1")
print(matrizconfusion1)
```
Tras los resultados, observamos que el modelo acierta 3 de cada 4 veces que predice (72.82%). Además, la tasa de verdaderos positivos es de 73.98%. 

Dado nuestro objetivo de minimizar los falsos positivos, este modelo no cumple demasiado bien los requisitos. A priori, no va a ser un buen modelo. Nos servirá como modelo base para intentar encontrar uno mejor.

Podríamos pensar que, aunque nuestro modelo no está siendo del todo bueno, alomejor sí que está siendo decisivo para algún rango concreto de probabilidades. Como tengo sólamente las siguientes probabilidades al ser k=3 (0, 0.3, 0.6,1), vamos a pintar la matriz de confusión con estos cortes: 

```{r}
predicciones <- predict(modelo1knn, newdata = scale(df[,-1]), type="prob")[,2]
table(cut(predicciones, c(-1, 0, 0.4,0.7,1)), as.factor(df$target))
```

Si observamos aquí nuestros datos, podríamos concluir que, si los 3 vecinos de nuestro nadador no bajan de marca, entonces está claro que él tampoco lo hace. De manera análoga, si los 3 vecinos lo hace, sí que baja de marca. Nuestro modelo está siendo efectivo en las probabilidades extremas, y entre medias, no tanto. Veamos ahora en test esto: 

```{r}
df <- datos2015Target_test %>% select(target, reactiontimeF, edad, pointP, reactiontimeP, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)

predicciones <- predict(modelo1knn, newdata = scale(df[,-1]), type="prob")[,2]
table(cut(predicciones, c(-1, 0, 0.4,0.7,1)), as.factor(df$target))
```

Parece que está sobreajustando y no es capaz de dividir igual de bien en test, luego no parece ser un buen modelo.
...
Para ello, volvemos a cargar todas las variables: 
...
El RFE seleccionará las variables más influyentes para predecir el objetivo (target) usando un Random Forest. Los resultados incluirán, tanto la lista de las mejores variables seleccionadas, como las métricas de rendimiento asociadas con cada tamaño de subconjunto.
...
print(matrizconfusion1)
```
El modelo clasifica correctamente el 78.97% de las observaciones. Casi 4 de cada 5, luego parece mejor que el primer modelo entrenado. Además, su tasa de verdaderos positivos es mayor que antes.

Probamos dividir nuestras predicciones en 4 grupos según sus probabilidades de mejora. De esta forma, intentamos reducir los errores de predicción para aquellos casos en los que no está realmente asegurada su mejora (o su no mejora). Para ello, realizamos la siguiente tabla, 

```{r}
predicciones <- predict(modelo2knn, newdata = scale(df[,-1]), type = "prob")[,2]
table(round(predicciones, 3),df$target)
```
Como vemos, dividiendo a nuestros nadadores en cuatro grupos, obtenemos que las predicciones para los participantes que mejoran coinciden en su totalidad con los datos de referencia. A su vez, las predicciones del primer grupo donde establecemos que no mejorarán su marca también carecen de error. 

Vistos los resultados del modelo en el conjunto de entrenamiento, esto nos dará una idea inicial del ajuste del modelo. Sin embargo, la eficacia de nuestro modelo radicará en cómo se comporta en datos no vistos.
...
matrizconfusion1 <- caret::confusionMatrix(predicciones, as.factor(df_test$target), positive = "1")
print(matrizconfusion1)

```

Vemos que, el Accuracy baja al predecir en datos nunca vistos, acierta un 65% de las veces que predice algo. Además, la tasa de verdaderos positivos es de un 61%, un poco baja. Aún así, este modelo es mejor que el anteriormente entrenado.

Si probamos en test nuestras predicciones con los umbrales impuestos anteriormente clasificando en varios grupos, tenemos: 

```{r}
predicciones <- predict(modelo2knn, newdata = scale(df_test[,-1]), type = "prob")[,2]
table(round(predicciones, 3),df_test$target)
```
Como vemos, tendríamos 4 grupos:

- Grupo 1: nadadores que con alta probabilidad podemos predecir que no mejorarán su marca. Acertamos en 4 de cada 5.
- Grupo 2: nadadores que probablemente, con menor seguridad, no mejorarán su marca. En este caso, acertamos en 6 de 8 participantes.
- Grupo 3: nadadores que posiblemente mejoren su marca. Acertamos en 14 de 23.
- Grupo 4: nadadores que con alta probabilidad mejorarán. Acertamos en 8 de 13. 

```{r}
rm(matrizconfusion1, df, df_test, predicciones)
```
...
matriz_confusion <- caret::confusionMatrix(as.factor(modelo3knn), as.factor(df_test$target), positive = "1")
...
Es un modelo muy parecido al modelo 2. Ya que la tasa de positivos verdaderos también es del 61%. Nos quedaremos con el segundo modelo. 


```{r}
#elimino el modelo porque el que me sirve es el modelo 2
rm(df,df_test,dist_train,dist_test,matriz_confusion, x_test, x_train, modelo3knn)
```
...
Basándonos en el resumen anterior presentado, optaremos por tomar de entre los K-NN el modelo 2.
...
plot.roc(roc_knn1,print.auc=T, print.thres = "best", col="blue",xlab = "Especificidad", ylab="Sensibilidad",xlim = c(1, 0), ylim = c(0, 1))

plot.roc(roc_knn2,print.auc=T, print.thres = "best", col="blue",xlab = "Especificidad", ylab="Sensibilidad",xlim = c(1, 0), ylim = c(0, 1))


# Calcular el AUC para cada modelo
auc_knn1 <- pROC::auc(roc_knn1)
auc_knn2<- pROC::auc(roc_knn2)
...
En este caso, aunque las curvas ROC nos proporcionan información, sería más útil trabajar en mejorar el modelo KNN antes de centrarnos en estas gráficas.


```{r, echo=FALSE}
rm(auc_knn1,auc_knn2,roc_knn1,roc_knn2,df_test1,df_test2, modelo1knn, x_train, modelo3knn, probs_knn1, probs_knn2, predicciones)
#Me guardo los dataframes para futuras comprobaciones más tarde: 

df_modelo2_knn<-datos2015Target_train %>% select(target, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)

df_modelo2_knn_test<-datos2015Target_test%>% select(target, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)

df_modelo2_knn_validation<-datos2015Target_validation%>% select(target, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)
```
...
## Además, definimos la variable distancia como categórica.
...
Nuestro árbol inicial presenta buenos resultados, con una precisión (Accuracy) del 81.1%, lo cual se considera un desempeño notable. Además, presenta también una buena precisión a la hora de identificar los nadadores que bajan de tiempo, un 81.67%. 
...
Para este modelo, usamos las mismas variables que en el modelo 1: 
...
En general, el modelo muestra un buen rendimiento: la precisión (87.69%) es bastante alta, lo que indica que la mayoría de las predicciones son correctas. Además, detecta bastante bien aquellos nadadores que consiguen bajar de marca, ya que consigue identificar el 84.92%. 


Lo analizamos a continuación con los datos test:
...
En este caso, tiene un accuracy de 57.14%, lo que indica que el modelo acierta en aproximadamente la mitad de las predicciones. Además, detecta 1 de cada 2 positivos aproximadamente. No es un buen modelo. 
...
#cambio de nombre
arbol3= arbol4
#no eliminamos arbolcv, lo usamos abajo
...
Otra manera de verlo sería a través de las curvas de ROC:

**¿se pueden sacar roc en arboles?**
```{r}
df_test_arbol <- datos2015Target_test %>% select(target, distance, stroke, gender, continente,heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, sesionP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)

df_test_arbol$distance<- as.factor(df_test_arbol$distance)

probs_arbol1 <- predict(arbol1, newdata = df_test_arbol, type = "prob")[, 2]

roc_arbol1 <- roc(df_test_arbol$target, probs_arbol1)


probs_arbol2 <- predict(arbol2, newdata = df_test_arbol, type = "prob")[, 2]

roc_arbol2 <- roc(df_test_arbol$target, probs_arbol2)

probs_arbol3 <- predict(arbol3, newdata = df_test_arbol, type = "prob")[, 2]

roc_arbol3 <- roc(df_test_arbol$target, probs_arbol3)

plot.roc(roc_arbol1, col = "blue", lwd = 2, main = "Curvas ROC de Modelos de Árboles de Decisión")
lines.roc(roc_arbol2, col = "red", lwd = 2)
lines.roc(roc_arbol3, col = "green", lwd = 2)

abline(a = 0, b = 1, col = "gray", lty = 2)


legend("bottomright", legend = c("Árbol 1", "Árbol 2", "Árbol 3"),
       col = c("blue", "red", "green"), lwd = 2, cex = 0.8)


auc_arbol1 <- pROC::auc(roc_arbol1)
auc_arbol2 <- pROC::auc(roc_arbol2)
auc_arbol3 <- pROC::auc(roc_arbol3)

cat("AUC Modelo 1:", auc_arbol1, "\n")
cat("AUC Modelo 2:", auc_arbol2, "\n")
cat("AUC Modelo 3:", auc_arbol3, "\n")

```

```{r}
rm(metricas, metricas_melt, auc_arbol1, auc_arbol2, auc_arbol3, probs_arbol1, probs_arbol2, probs_arbol3, roc_arbol1, roc_arbol2, roc_arbol3, df_test_arbol)
```
...
En este paso, utilizaremos el modelo 2 basado en árboles de decisión como modelo base para implementar un modelo de Bagging. El objetivo es mejorar la generalización y la precisión del modelo al combinar múltiples árboles entrenados en subconjuntos de datos de entrenamiento.
...
Una vez entrenado el modelo de Bagging, evaluamos su desempeño en el conjunto de entrenamiento utilizando la función predict para obtener las probabilidades de cada clase. A continuación, transformamos estas probabilidades en predicciones binarias (0 o 1) con un umbral del 50%. Finalmente, calculamos la matriz de confusión y otras métricas.
...
```{r}
rm(cf_bag_train, cf_bag_test, clase.pred.bag.test, clase.pred.bag.train, pred_bag_test, pred_bag_train)
```
...
clase.pred.bag.train=ifelse(pred_bag_train>0.5,"1","0")
cf_bag_train <- caret::confusionMatrix(as.factor(clase.pred.bag.train), as.factor(df_arbol$target),positive="1")
...
clase.pred.bag.test=ifelse(pred_bag_test>0.5,"1","0")
cf_bag_test <- caret::confusionMatrix(as.factor(clase.pred.bag.test), as.factor(df_test_arbol$target),positive="1")

cf_bag_test
```
El modelo muestra un claro sobreajuste. Si bien funciona perfectamente en el conjunto de entrenamiento, su rendimiento en el conjunto de prueba es peor, lo que sugiere que puede no estar generalizando bien. Sin embargo, se comporta ligeramente mejor que en el caso anterior.
...
cf_bag_train
```
Probamos a cortar en 3 categorías con el fin de reducir los errores de predicción:

```{r}
# Cortar las probabilidades en categorías
pred_bag_train_cut <- cut(pred_bag_train, breaks = c(-Inf, 0.30, 0.70, Inf), labels = c("Low", "Medium", "High"))

# Crear la tabla de contingencia
tabla_resultados <- table(pred_bag_train_cut, df_arbol$target)
...
Vemos ahora cómo se comporta el modelo en los datos test:
...
cf_bag_test
```
Los resultados indican que el modelo clasifica correctamente el 63.27% de los casos en el conjunto de prueba, lo cual es una mejora respecto a algunos de los modelos previos.

Por otro lado, no evalúa demasiado bien cuando predice que un nadador mejora, es decir, predice bien 6 de cada 10 nadadores que mejoran. Aún así es mucho mejor modelo que los anteriores, lo tendremos en cuenta.

Si probamos el modelo respecto a los tres grupos: muy probable, igualmente probable y poco probable para las predicciones de nuestra variable target. 

```{r}
# Cortar las probabilidades en categorías
pred_bag_test_cut <- cut(pred_bag_test, breaks = c(-Inf, 0.30, 0.70, Inf), labels = c("Low", "Medium", "High"))

# Crear la tabla de contingencia
tabla_resultados <- table(pred_bag_test_cut, df_test_arbol$target)

# Mostrar la tabla
print(tabla_resultados)
```

De esta forma tenemos que para el primer grupo (personas que no mejoran su marca), el modelo acierta 8 de cada 12 nadadores (0.66%) y para el tercer grupo (personas que mejoran su marca), el modelo acierta 15 de cada 24 (62,5%).


```{r}
rm(cf_bag_train, cf_bag_test, tabla_resultados, pred_bag_train_cut, pred_bag_train, pred_bag_test_cut, pred_bag_test, clase.pred.bag.test, clase.pred.bag.train)
```
...
Utilizamos un conjunto de hiperparámetros básicos para construir múltiples árboles de decisión.

Se logró un desempeño perfecto en el conjunto de entrenamiento, pero esto fue un indicio de sobreajuste.

En el conjunto de prueba, el modelo mostró un buen desempeño en sensibilidad (identificar nadadores que mejoran), pero una baja especificidad (alto número de falsos positivos).


2. Modelo 2: 

Se ajustaron los hiperparámetros del modelo base, aumentando el tamaño mínimo de división de nodos y la profundidad de los árboles.

Aunque mejoró ligeramente respecto al Modelo 1, siguió mostrando un rendimiento pobre en el conjunto de prueba debido al sobreajuste.

3. Modelo 3:

Este modelo utilizó parámetros optimizados y seleccionó las variables más relevantes del análisis previo.

Mejoró significativamente en términos de generalización, alcanzando una precisión del 63.27% en el conjunto de prueba.

Logró clasificar correctamente a 6 de cada 10 nadadores que mejoraron, lo que lo hace superior al primer modelo.

Lo vemos gráficamente:

```{r}
probs_bag1 <- predict(modeloBagging1, df_test_arbol, type = "prob")[, 2]
probs_bag2 <- predict(modeloBagging2, df_test_arbol, type = "prob")[, 2]
probs_bag3 <- predict(modeloBagging3, df_test_arbol, type = "prob")[, 2]

roc_bag1 <- roc(df_test_arbol$target, probs_bag1)
roc_bag2 <- roc(df_test_arbol$target, probs_bag2)
roc_bag3 <- roc(df_test_arbol$target, probs_bag3)

auc_bag1 <- pROC::auc(roc_bag1)
auc_bag2 <- pROC::auc(roc_bag2)
auc_bag3 <- pROC::auc(roc_bag3)


cat("AUC Bagging modelo 1:", auc_bag1, "\n")
cat("AUC Bagging modelo 2:", auc_bag2, "\n")
cat("AUC Bagging modelo 3:", auc_bag3, "\n")

# Graficar las curvas ROC
plot.roc(roc_bag1, col = "blue", lwd = 2, main = "Comparación de Curvas ROC para Modelos Bagging",
         xlab = "Especificidad", ylab = "Sensibilidad", xlim = c(1, 0), ylim = c(0, 1))
lines.roc(roc_bag2, col = "red", lwd = 2)
lines.roc(roc_bag3, col = "green", lwd = 2)

abline(a = 0, b = 1, col = "gray", lty = 2)

#leyenda
legend("bottomright", legend = c("Bagging modelo 1", "Bagging modelo 2", "Bagging modelo 3"),
       col = c("blue", "red", "green"), lwd = 2, cex = 0.8)


```

El Modelo 2 es el mejor modelo en cuanto a capacidad discriminativa, con un AUC de 0.6475.

Quedarse con el Modelo 2 es una posible elección , ya que tiene el mayor AUC (0.6475), lo que indica una mejor capacidad discriminativa global.

Aunque el Modelo 3 tiene una mejor interpretación en un contexto práctico específico, el Modelo 2 demuestra un rendimiento más sólido a nivel general.

```{r}
#Elimino todo lo que no usaré en adelante.
#Elimino el modelo 1 ya que no va a ser relevante. 
rm(modeloBagging1, roc_bag1, roc_bag2, roc_bag3, auc_bag1, auc_bag2, auc_bag3, pred_bag_test, pred_bag_train, clase.pred.bag.test, clase.pred.bag.train, probs_bag1, probs_bag2, probs_bag3)

df_baggin_train<-df_arbol
df_baggin_test<-df_test_arbol
df_baggin_validation<-datos2015Target_validation %>% select(target, distance, stroke, gender, name,heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)

rm(df_arbol, df_test_arbol)
```
...
Vamos a entrenar un primer modelo de Random Forest. Para ello, voy a recoger todas las variables posibles. Son las mismas variables que se usaban para los árboles de decisión. 
...
Una vez recogidas las variables en el dataframe, vamos a proceder a entrenar nuestro primer modelo de Random Forest (rf1):
...
A continuación, graficamos el error: 
...
A continuación, vamos a realizar las distintas predicciones en nuestro conjunto de entrenamiento: 
...
Observamos que el modelo no comete errores en el conjunto de entrenamiento, identificando perfectamente todos los patrones. Esto sugiere la presencia de sobreajuste, tal y como hemos aprendido a lo largo del proyecto. Visualizamos a continuación los datos en test: 
...
Tal y como preveíamos, el modelo se ha sobreajustado a los datos de entrenamiento, lo que lo hace poco válido en la práctica. Su capacidad de generalización es limitada. 
...
```
Realizamos un análisis de la importancia de las variables en el modelo Random Forest. Tenemos como medidas "MeanDecreaseAccuracy" y "MeanDecreaseGini". 

  - La "MeanDecreaseAccuracy" mide la reducción en la precisión del modelo cuando se elimina o cambia una variable. Evalúa cuánto empeora la precisión del modelo si la variable en cuestión no está disponible para hacer predicciones. Un valor alto de MeanDecreaseAccuracy indica que, al eliminar esa variable, la precisión del modelo disminuye considerablemente, lo que significa que la variable es importante para el rendimiento del modelo
  
  - La "MeanDecreaseGini" se basa en la impureza de Gini, es una medida de cuán mezcladas están las clases en un nodo del árbol de decisión. Cuanto mayor es el valor para una variable, mas contribuye a la capacidad del modelo para separar correctamente las clases y mejorar las predicciones. Refleja cuanto mejora la capacidad del modelo para hacer predicciones correctas al incluir esa variable en el modelo. Si es más alto, mayor importancia tiene dicha variable ya que ayuda a dividir mejor los datos en los árboles del modelo.
  
Si tomamos nuestros resultados, tenemos que  variables como laneF, laneP, swimtimeP, pointP y daytimeP destacan con altos valores en MeanDecreaseGini, lo que sugiere que son las más relevantes para el modelo. Sin embargo, algunas variables, como heatF y otras con valores cercanos a cero o negativos, no aportan información significativa y podrían considerarse menos importantes. La presencia de valores negativos en MeanDecreaseAccuracy sugiere que algunas variables podrían introducir ruido o no ser informativas para el modelo.
...
Realizamos un segundo modelo con el objetivo de mejorar el rendimiento del Random Forest anterior. 
...
Generamos el gráfico de los errores de clasificación a lo largo del número de árboles en el modelo Random Forest. Se muestra cómo evoluciona el error (tanto general como para cada clase, si es una tarea de clasificación) a medida que se agregan más árboles.
...
Implementamos la predicción con el modelo de Random Forest (rf2) en los conjuntos de train y test. Ajustamos el umbral de decisión a 0.66.

```{r}

# Predicción en el conjunto de entrenamiento
pred_rf_train <- predict(rf2, df_random, type = "prob")[, 2]  
clase_pred_rf_train <- ifelse(pred_rf_train > 0.7, "1", "0")  
cf_rf_train <- caret::confusionMatrix(as.factor(clase_pred_rf_train), as.factor(df_random$target), positive = "1")
print(cf_rf_train)
```

Tras varias pruebas en consola, hemos observado que el mejor valor que discriminaba las dos categorías es con 0.7 como corte. 
Obtenemos medidas de rendimiento prácticamente perfectas. Vamos a realizar también su ajuste en 3 cortes, con el objetivo de tener mejores predicciones en test.

```{r}
# Obtener las probabilidades de predicción
pred_rf_train <- predict(rf2, df_random, type = "prob")[, 2] 

# Dividir en 3 grupos según los cortes: 0-0.50, 0.50-0.7 y 0.7-1
clase_pred_rf_train <- cut(pred_rf_train, 
                          breaks = c(-Inf, 0.50, 0.7, Inf), 
                          labels = c("0", "Medium", "1"))

table(clase_pred_rf_train, df_random$target)

```

Evaluamos en test el posible sobreajuste de este modelo y si realmente es mejor o peor establecer dos dos umbrales adicionales.
...
clase_pred_rf_test <- ifelse(pred_rf_test > 0.7, "1","0")   

cf_rf_test <- caret::confusionMatrix(as.factor(clase_pred_rf_test), as.factor(df_test_random$target), positive = "1")
print(cf_rf_test)
```

El rendimiento del modelo ha mejorado en comparación con el random forest anterior. Este modelo presenta mejor desempeño en la identificación de negativos (especificidad) y ligeras mejoras en la exactitud.

A continuación, comprobamos en test creando disintas clases dependiendo la probabilidad.
...
# Dividir en 3 grupos según los cortes: 0-0.50, 0.50-0.7 y 0.7-1
clase_pred_rf_test <- cut(pred_rf_test, 
                          breaks = c(-Inf, 0.50, 0.7, Inf), 
                          labels = c("0", "Medium", "1"))

table(clase_pred_rf_test, df_test_random$target)

```

De esta forma, dejamos sin clasificar positiva o negativamente a 15 personas, las posicionamos en "incertidumbre". Esto nos reduce el error de falsos negativos de un 38,8% a un 33,33%. El umbral para afirmar que el nadador bajará de tiempo es el mismo. Por lo que en este sentido no cambia. 


```{r}
rm(pred_rf_train, clase_pred_rf_train,cf_rf_train,pred_rf_test,clase_pred_rf_test ,cf_rf_test,roc_curve,best_threshold )
```
...
preProc <- preProcess(df_test_random[, -which(names(df_test_random) == "target","distance")], 
                      method = c("center", "scale"))
df_random_test_std<- predict(preProc, df_test_random)
...
# Predicción en el conjunto de entrenamiento
pred_rf_train <- predict(rf_model, df_random_std, type = "prob")[, 2]
clase_pred_rf_train <- factor(ifelse(pred_rf_train > 0.5, "1", "0"), levels = c("0", "1"))

# Matriz de confusión para el conjunto de entrenamiento
cf_rf_train <- caret::confusionMatrix(clase_pred_rf_train, df_random_std$target, positive= "1")
print(cf_rf_train)

# Predicción en el conjunto de prueba
pred_rf_test <- predict(rf_model, df_random_test_std, type = "prob")[, 2]
clase_pred_rf_test <- factor(ifelse(pred_rf_test > 0.5, "1", "0"), levels = c("0", "1"))

# Matriz de confusión para el conjunto de prueba
cf_rf_test <- caret::confusionMatrix(clase_pred_rf_test, as.factor(df_random_test_std$target),positive="1")
print(cf_rf_test)

```

Tenemos una precisión en torno al 53%. Sin embargo, si nuestro objetivo es minimizar los falsos positivos este es el mejor modelo. Teniendo como criterio de medida la especificidad, obtenemos un 91% en los datos de test. Podría considerarse un buen modelo para nuestro objetivo porque, si predice que mejora, realmente hay una gran probabilidad de acierto.


```{r}
rm(pred_rf_test, pred_rf_train, clase_pred_rf_test, clase_pred_rf_train, train_control, tune_grid, preProc, cf_rf_test, cf_rf_train)
```
...
Se entrenó utilizando todas las variables disponibles y una configuración básica con 20 árboles y 6 variables por división.

Su desempeño fue perfecto en el conjunto de entrenamiento, lo que indica sobreajuste.
En el conjunto de prueba, el rendimiento fue bajo, mostrando mala generalización.

Las variables más importantes según "MeanDecreaseAccuracy" y "MeanDecreaseGini" fueron laneF, laneP, swimtimeP, pointP, y daytimeP.

El modelo tiene limitaciones significativas debido al sobreajuste y no es apto para generalización en nuevos datos.

2. Random Forest 2:

Ajustamos el modelo básico aumentando el número de árboles (ntree = 90) y variables por división,  además de definir un umbral óptimo para maximizar el rendimiento.

Obtuvimos una mejor capacidad de generalización en comparación con el Modelo 1.

Ajustando el umbral a 0.7, se observó una mejora en la especificidad (capacidad para identificar correctamente los negativos).

Una estrategia adicional dividió los resultados en tres grupos: probabilidad baja, media e incierta. Esto redujo el error en falsos negativos (de 38.8% a 33.33%). Este modelo es más balanceado y logra un mejor compromiso entre especificidad y sensibilidad.

3. Random Forest 3: 

Se incluyó estandarización de datos, validación cruzada, y ajustes en parámetros como ntree = 200, mtry entre 3 y 9, y aumento del tamaño mínimo de nodos terminales. En este caso, buscamos optimizar la especificidad.

Aunque no es el más preciso, la especificidad es alta.

```{r}

pred_rf_test1 <- predict(rf1, df_test_random, type = "prob")[, 2]
roc_rf1 <- roc(df_test_random$target, pred_rf_test1)

pred_rf_test2 <- predict(rf2, df_test_random, type = "prob")[, 2]
roc_rf2 <- roc(df_test_random$target, pred_rf_test2)

pred_rf_test3 <- predict(rf_model, df_random_test_std, type = "prob")[, 2]
roc_rf3 <- roc(df_test_random$target, pred_rf_test3)


auc_rf1 <- pROC::auc(roc_rf1)
auc_rf2 <- pROC::auc(roc_rf2)
auc_rf3 <- pROC::auc(roc_rf3)

# Imprimir los valores de AUC
cat("AUC Modelo 1:", auc_rf1, "\n")
cat("AUC Modelo 2:", auc_rf2, "\n")
cat("AUC Modelo 3:", auc_rf3, "\n")

# Graficar las curvas ROC
plot.roc(roc_rf1, col = "blue", lwd = 2, main = "Comparación de Curvas ROC - Random Forest",
     xlab = "Especificidad", ylab = "Sensibilidad", xlim = c(1, 0), ylim = c(0, 1))
lines(roc_rf2, col = "red", lwd = 2)
lines(roc_rf3, col = "green", lwd = 2)

# Línea de referencia
abline(a = 0, b = 1, col = "gray", lty = 2)

# Leyenda
legend("bottomright", legend = c("Modelo 1 RF", "Modelo 2 RF", "Modelo 3 RF"),
       col = c("blue", "red", "green"), lwd = 2, cex = 0.8)

```
En definitiva, el Modelo 2 es el mejor de los tres, al tener un mayor valor de AUC, lo que indica un mejor desempeño global. Presenta un balance adecuado entre sensibilidad y especificidad, especialmente tras ajustar el umbral de decisión. Es un modelo más robusto en términos de generalización, con menos riesgo de sobreajuste que el Modelo 1 y mejores predicciones globales que el Modelo 3.



```{r}
rm(auc_rf1, auc_rf2, auc_rf3, pred_rf_test1, pred_rf_test2, pred_rf_test3, roc_rf3, roc_rf2, roc_rf1)
```
...
Una de las grandes ventajas de esta técnica es su interpretabilidad global y local.

- Global: Muestra cuáles son las características más importantes a nivel general en el modelo.

- Local: Explica por qué el modelo hizo una predicción específica para un caso individual.

Para entender correctamente qué características han contribuido predominantemente en nuestros modelos, veremos en primer lugar la técnica SHAP con el Random Forest de 4 variables. Posteriormente lo veremos con el Random Forest optimizado según su especificidad. 

## Uso en modelo 2 (RF). 

Elegimos el modelo de Random Forest y creamos un predictor. Este predictor conectará la técnica SHAP con el modelo que hemos elegido, separando la variable objetivo de las explicativas. 

```{r}
# Crear un predictor para el modelo
predictor_rf2 <- Predictor$new(
  model = rf2,
  data = df_random[,-1],  # Variables explicativas
  y = as.factor(df_random$target)                                                  # Variable objetivo
)
...
A continuación, elegimos una fila específica para calcular y ver sus valores SHAP. Pero antes de ello, voy a ver qué predicción hace mi modelo para la primera fila: 

```{r}
predict(rf2, df_random[1,], type = "prob")[,2]
```

Luego, es una fila a la que le predice que NO baja de tiempo. Lo tendremos en cuenta a la hora de interpretar el siguiente gráfico.

```{r}
# Calcular valores SHAP para una observación específica (primera fila)
shap_rf2 <- Shapley$new(
  predictor = predictor_rf2,
  x.interest = df_random[1,-1]
)
...
SHAP evalúa el modelo para todas las combinaciones posibles de variables (introduciéndolas o desechándolas). Luego el método promedia estas contribuciones marginales para determinar el impacto de cada característica. De esta forma, tenemos la interpretación local de la primera predicción.
...
```{r}
rm(predictor_rf2, shap_rf2)
```

## SHAP para Random Forest 3

Tras ver las contribuciones en el Random Forest de 4 variables, optamos por realizar esta técnica en el Random Forest que incluye todas las variables escaladas. En este caso, será más interpretable puesto que el Random Forest ha sido creado mediante muchos árboles y los parámetros han sido elegidos por optimización. Por tanto, de esta forma podemos ver a nivel general las contribuciones de cada variable. 

```{r}
# Crear un predictor para el modelo
predictor_rf_model <- Predictor$new(
  model = rf_model,
  data = df_random_std, 
  y = df_random_std$target  # Variable objetivo
)
...
```{r}
# Calcular valores SHAP para una observación específica (primera fila)
shap_rf_model <- Shapley$new(
  predictor = predictor_rf_model,
  x.interest = df_random_std[1, ]
)

# Mostrar los valores SHAP calculados
shap_rf_model$results
plot(shap_rf_model)
```
Vemos en este caso que las variables que han aportado mayor contribución a la predicción son: tipo de nado, minimo parciales, media parciales, mediana de los parciales y las calles preliminares y finales. 

```{r}
# Calcular valores SHAP para múltiples observaciones
shap_rf_model_global <- FeatureImp$new(predictor_rf_model, loss = "ce")

# Gráfico de importancia global basado en SHAP
plot(shap_rf_model_global)

```
A nivel global, se observa que las características más influyentes en la predicción son la calle en preliminares y finales, el tiempo total y el estilo de nado. Estas variables son clave para la predicción, ya que, como se observó en un análisis preliminar, ciertos estilos de nado tienden a tener una mayor probabilidad de mejorar en el tiempo final que otros. Además, es razonable suponer que el tiempo en los parciales puede ofrecer una buena indicación del rendimiento en la ronda final. Finalmente, la calle utilizada en la final y semifinal no es seleccionada al azar; los nadadores con mejores tiempos suelen ser asignados a las calles intermedias, lo que introduce un sesgo en esta variable.


```{r}
rm(predictor_rf_model, shap_rf_model, shap_rf_model_global)
```
...
```{r}
probs_arbol <- predict(arbol2, newdata = df_test_arbol[,-1], type = "prob")[, 2]

probs_knn2<- predict(modelo2knn, newdata = df_modelo2_knn_test[,-1], type = "prob")[, 2]

probs_bag2 <- predict(modeloBagging2, df_baggin_test, type = "prob")[, 2]

probs_random<-predict(rf2, newdata = df_test_random[,-1], type = "prob")[, 2]

roc_1 <- roc(df_test_arbol$target, probs_arbol)
roc_2<- roc(df_modelo2_knn_test$target, probs_knn2)
roc_3<- roc(df_test_random$target, probs_random)
roc_4<- roc(df_baggin_test$target, probs_bag2)
#mirar los dataframes. 

# Calcular el AUC para cada modelo
auc_1 <- pROC::auc(roc_1)
auc_2<- pROC::auc(roc_2)
auc_3<- pROC::auc(roc_3)
auc_4<- pROC::auc(roc_4)

# Imprimir los valores de AUC
cat("AUC  Árbol", auc_1, "\n")
cat("AUC KNN :", auc_2, "\n")
cat("AUC RF:", auc_3, "\n")
cat("AUC Bagging:", auc_4, "\n")

#cat("AUC KNN sin escalar k=3:", auc_knn_es_3, "\n")
#cat("AUC KNN con variables seleccionadas:", auc_knn_selected, "\n")

# Graficar las curvas ROC
plot(roc_1, col = "blue", lwd = 2, main = "Comparación de Curvas ROC para los Mejores Modelos")
lines(roc_2, col = "red", lwd = 2)
lines(roc_3, col = "pink", lwd = 2)
lines(roc_4, col = "yellow", lwd = 2)

abline(a = 0, b = 1, col = "gray", lty = 2)

legend("bottomright", legend = c("Árbol", "Knn", "Random Forest","Bagging"),
       col = c("blue", "red", "pink","yellow"), lwd = 2, cex = 0.5) 
```
Teniendo en cuenta los resultados en cada uno de los modelos ajustados y las curvas ROC, tomamos el modelo Bagging como el mejor modelo para predecir nuestra variable objetivo. 


# Validación

```{r}
validation <- datos2015Target_validation %>% select(target, distance, stroke, gender,continente, heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)
validation$distance<-as.factor(validation$distance)
```
...
```{r}
library(caret)
pred_rf <- predict(rf2, validation[,-1] ,type="class")

cf_rf_val <- caret::confusionMatrix(pred_rf, as.factor(validation$target), positive='1')
cf_rf_val
```

El modelo clasifica correctamente el 66.67% de los casos (Accuracy) y al 72.22% de los casos positivos (Sensitivity). El 76.47% de las veces que predice un caso como positivo son correctas. La especificidad es relativamente baja (55.56%), lo que indica que el modelo tiene problemas para identificar correctamente la clase negativa. 
Sin embargo, debemos tener en cuenta que el conjunto de datos en validación es limitado. Además, hemos de tener en cuenta que la tasa de falsos positivos es relativamente baja, el modelo captura en su mayoría correctamente a los casos positivos. 
