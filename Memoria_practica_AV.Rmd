...
## Modelo 1.
Primeramente, vamos a probar a introducir todas las variables continuas posibles y escaladas. Usaremos un modelo knn con definido con distancias euclídeas: 

```{r}
# Elijo todas las variables continuas.
...
Bien, una vez entrenado, veamos el resultado: 
...
Ya en nuestros datos de entrenamiento está fallando demasiado, luego no va a ser un buen modelo. Aún así, será nuestro modelo base para intentar encontrar uno mejor. 
...
Ahora, hacemos una selección de las mejores variables que deberían entrar. Para ello, volvemos a cargar todas las variables categóricas: 
...
Bien, una vez cargadas, vamos a usar una función de la librería caret que de cierta manera, elegirá las mejores variables para el modelo: 

```{r}
x <- scale(df[,-1])
y <- df$target  # Tu variable objetivo
...
Bien, luego parece que el mejor modelo considera meter swimtimeP, medianaParcialesP, maximoParcialesP, minimoParcialesP y mediaParcialesP. Veamos cómo se ajusta este modelo: 
...
Una vez entrenado mi modelo, vamos a ver cuál es su rendimiento en train: 
...
Parece que nuestro modelo mejora en train respecto al anterior. Veamos cómo se comporta en test: 
...
Nos sale un modelo con un 0.65 en test. (comentar luego)
...
En este modelo, usaremos la distancia de Gower, una métrica que permite calcular distancias entre observaciones con datos mixtos (categóricos y numéricos). 

Voy a realizar la misma selección de variables que en el modelo anterior.
```{r}
df <- datos2015Target_train %>% select(target, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)
...
Calculamos las distancias de Gower en nuestros datos(ya está implementada en R)
```{r}
dist_train <- gower.dist(df[, -1])  
dist_test <- gower.dist(df_test[, -1])  

# otra opción de distancias para quien quiera mirarlo
#dist_train <- proxy::dist(df[, -1], method = "manhattan")
#dist_test <- proxy::dist(df_test[, -1],df[, -1], method = "manhattan")

```

Miraré próximamente si es posible 
```{r}
...
#predicciones <- predict(modelo3knn, newdata = scale(df[,-1]), type = "class")
#matrizconfusion1 <- confusionMatrix(predicciones, as.factor(df$target), positive = "1")
#print(matrizconfusion1)
...
```{r}
#rm(df,df_test,dist_train,dist_test)
```



## Modelo 4. (si da tiempo)

En este apartado, tratar de hacer un modelo definiendo distancias entre obsevaciones de alguna manera. (seguramente sea el más complicado y lioso de hacer, dejarlo para el final)
...
Vamos a comenzar creando un árbol con todas las variables, aunque vamos a modificar nuestra variable code (país), por el continente para tratar de generalizar nuestro árbol. Además, cambiaremos daytimeP por la sesion (matinal o vespertina). V
...
## Modelo 3.
En este modelo vamos a jugar con el cp. Escogiendo las variables que mas me interesan.
...
```
Además, veo la importancia de las variables.
```{r}
...
rm(cf_train, cf_test, prediction,importancia,control)
```


CONCLUSION hasta el momento ARBOLES PARA EL BAGGING(14/12): el que mejor va es arbol3
...
Voy a meter exactamente los mismos parámetros de control que empleábamos en el arbol3 (el que mejor iba y veo que pasa)

Además, recordamos que hacíamos selección de variables(realizamos la misma que en arbol3)
```{r}
# Bagging
set.seed(123)
...
Lo hemos mejorado. De hecho, sin selección empeora.
...
## Modelo 2
Realmente con un buen Random hemos ganado y te realiza seleccion, pero realizo selección de variables manual por si algún casual mejora(me baso en mi desinformación creo q esto no es necesario)
...
### Manera 1
(implementado con R)
Lo voy a hacer sobre el rf1(opción barajada de cambios ante un mejor rf). 
...
```
Para saber lo que queremos mejorar, vemos la predicción de la probabilidad de cada clase.
```{r}
# Predicción de la probabilidad de cada clase
predictor$predict(x_interest) # probability of being a good credit risk

```

Ahora estudiamos qué factores  se deben cambiar para que la probabilidad de mejora sea de al menos un 80%.
fixed_features aquellos que no vamos a poder cambiar, voy a poner la edad, el continente y el género(Alonso con tu conosimiento se fija alguno mas?)
```{r}
head(df_random)
```
...
cfactuals = moc_classif$find_counterfactuals(
   x_interest, desired_class = "1", desired_prob = c(0.8, 1))


```


Observamos los contrafácticos logrados(son 85). Vemos los 3 primeros
...
```
Lo que más cambia es LaneF y dayTimeP en los contrafácticos. Realizaré una interpretación.
