...
Obtenemos medidas de rendimiento prácticamente perfectas. Evaluamos en test el posible sobreajuste de este modelo.
```{r}
roc_curve <- roc(df_random$target, pred_rf_train)
...
#pred_bag <- predict(modeloBagging2, df_final ,type="prob")[,2]
#clase.pred.bag.test=ifelse(pred_bag_test>0.5,"1","0")
#pred_bag_validation <- predict(modeloBagging2, df_final,type="prob")[,2]
#clase.pred.bag.validation=ifelse(pred_bag_validation>0.5,"1","0")

#confusionMatrix(pred_bag_validation)
```
```{r}
predicciones <- predict(modeloBagging2, newdata = df_final, type = "class")
#matrizconfusion <- caret::confusionMatrix(predicciones, as.factor(df_final$target), positive = "1")
