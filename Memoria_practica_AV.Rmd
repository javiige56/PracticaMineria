...
Basándonos en el resumen anterior presentad, tenemos 2 modelos principales (modelo1knn y modelo2knn)
...
Este modelo representa un intento de mejorar el rendimiento de un Random Forest ajustado previamente. 
...
Este comando genera un gráfico de los errores de clasificación a lo largo del número de árboles en el modelo Random Forest. Este gráfico muestra cómo evoluciona el error (tanto general como para cada clase, si es una tarea de clasificación) a medida que se agregan más árboles.
...
```
Implementamos la predicción con un modelo de Random Forest (rf2) en los conjuntos de entrenamiento y prueba, ajustando un umbral de decisión para clasificar las probabilidades en clases. Además, calculamos y mostramos matrices de confusión para evaluar el rendimiento. Aquí tienes una explicación detallada y posibles mejoras:
...
```
El rendimiento del modelo ha mejorado en comparación con el análisis previo.Mejor desempeño en la identificación de negativos (especificidad). Ligeras mejoras en la exactitud.
...
Parece que hemos maximizado la especificidad gracias a este cambio. 
En este, nuestro mejor Random Forest, realizamos cortes **AÑADIR LO DE ISAAC A ESTE**.
...
La gran mayoría de contrafácticos filtrados nadan la final en la calle 8. Nuestro nadador de interés nadó en la calle 4. Me indica que en la preliminar debería haber nadado más rápido. c
