...
Observamos los resultados del modelo en el conjunto de entrenamiento. Esto nos da una idea inicial del ajuste del modelo. Sin embargo, lo más importante es cómo se comporta en datos no vistos:
...
```{r}
predicciones <- modelo3knn
matriz_confusion <- confusionMatrix(as.factor(predicciones), as.factor(df_test$target), positive = "1")

# Mostrar la matriz de confusión y métricas
print(matriz_confusion)
```
Un 65.31% de las predicciones fueron correctas. Balance aceptable entre sensibilidad y especificidad.



```{r}
rm(df,df_test,dist_train,dist_test,matriz_confusion, x_train,x_test,predicciones)
```
...
También se podria mirar la distancia de Gower
...
MODIFICAR ESTO ADAPTANDOSE A LOS MODELOS ACTUALES
...
**DESDE AQUÍ EMPIEZA EL RESUMEN ACTUAL:**

Basándome en los resultados que se presentaron, tenemos 2 modelos principales(modelo1knn y modelo2knn)
...
Aunque el AUC del modelo 2 es superior, un valor de 0.5275 sigue siendo bajo. Esto sugiere que ambos modelos tienen un desempeño moderado y que todavía hay espacio para mejorar, ya sea ajustando hiperparámetros (k), probando otras distancias (como Manhattan o Gower), o incluyendo nuevas variables predictoras.
...
## Modelo 3.
En este modelo vamos a jugar con el cp. Escogiendo las variables que mas me interesan(evidentemente esto ya lo esta realizando el árbol, es como si estuviesemos cortando a cierta profundidad,ANALOGO)
