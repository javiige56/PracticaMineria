...
```{r, echo=FALSE}
rm( conteo_por_continente, datos_100m_top, grafico_distribucion_continente, histograma_nadadores, manual, puntos_por_continente, puntos_por_pais, resumen_paises, resumenContinente, nombres, paleta)
...
El método de los k vecinos más cercanos (abreviatura de “k nearest neighbors” en inglés), se cuenta entre los enfoques más simples y ampliamente utilizados en el campo del Machine Learning.
Este método se basa en la noción de similitud (o distancia) entre observaciones.

En un problema de clasificación, el algoritmo devolverá la clase que predomina entre los vecinos, es decir, la moda de la variable objetivo.
...
Primeramente, vamos a probar introduciendo variables que razonablemente podrían determinar los mejores vecinos.
Es importante que las variables seleccionadas sean informativas y relevantes para predecir el comportamiento objetivo.
Las variables introducidas, son las siguientes: - athelteid.
Esta variable podría ser útil si hay patrones repetitivos en el rendimiento de un mismo nadador a lo largo de las pruebas.

-   eventidF.
    Es el id de evento de la final.
    Al introducir esta variable no es necesario introducir otras como son: el sexo, estilo y distancia, al agrupar a los nadadores por la misma competencia.

-   reactionTimeF.
    Supondremos que vamos a predecir si los nadadores bajan justo en el momento en el que saltan.
    Esta variable es relevante para predecir si un nadador bajará su tiempo.
    Si su tiempo de reacción en la final es excesivamente alto, podría influir en la decisión de nuestro modelo.

-   edad.
    Puede ser importante, ya que la edad puede influir en el rendimiento, tal y como hemos observado previamente.

-   eventidP.
    Similar al EventidF, el evento preliminar puede agrupar a los nadadores que compiten en las mismas condiciones.

-   pointP.
    Puntos de las preliminares.

-   reactionTimeP.
    Tiempo de reacción de la prueba previa.

-   swimtimeP.
    Tiempo final de la prueba previa.

-   daytimeP.
    Hora de la preliminar.

-   mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP.
    Son bastante útiles, ya que proporcionan información más detallada sobre el rendimiento del nadador a lo largo de la prueba.
    El promedio de los parciales puede indicar la consistencia del nadador.
    El máximo, el mínimo, la media y la mediana pueden ayudar a identificar vecinos con características parecidas sobre cómo plantean la prueba.

A continuación, entrenamos un primer modelo fijándonos en los 5 vecinos más proximos con todas las variables y usando la distancia euclídea entre observaciones sin escalar los datos:
...
Observamos el rendimiento del modelo en el conjunto de datos de entrenamiento,
...
La precisión del modelo está en un 57%.
Es decir, acierta prácticamente por azar.
Por tanto,no es un buen modelo.
Probaremos con algunas modificaciones esperando conseguir un modelo mejor: escalar, introducir o descartar variables y/o elegir un k distinto.
...
Dado que KNN depende de la distancia entre las observaciones, las variables con rangos más amplios podrían influir desproporcionadamente en los resultados.
Por lo tanto, escalaremos antes de aplicar KNN.
De esta forma, todas las variables contribuirán equitativamente.
...
Con el objetivo de obtener un modelo más preciso, utilizaremos la validación cruzada para encontrar el mejor k.
Es decir, el número ideal de observaciones más cercanas (vecinos) en las que basar nuestras predicciones.
...
Tenemos en los resultados que el mejor k es 3.
Por tanto, evaluamos el modelo con k=3 y las variables escaladas.
Posteriomente, entrenamos el modelo y analizamos la matriz de confusión en los datos de entrenamiento.
...
Parece que se produce una mejora respecto al anterior modelo tanto en accuracy como en sensitivity.
A continuación, evaluamos el modelo en el conjunto de test.
Para ello, escalamos los datos test.

```{r}
df_test_scaled <- predict(preProcValues, df_test %>% select(-target))  # Escalar todas menos el target
```

Tras este paso, generamos las predicciones.
...
El modelo tiene un buen rendimiento en el conjunto de entrenamiento con una precisión y especificidad alta (78.97% y 69.88%, respectivamente).
Esto indica que el modelo está capturando patrones en los datos de entrenamiento.
Sin embargo, en el test ambas caen, 53.06% (accuracy), 33.33% (especificidad), lo que sugiere un sobreajuste a mis datos de entrenamiento.
...
A continuación, probamos a usar k=3 con los datos sin escalar.
Comenzamos nuevamente seleccionando las variables.
...
A continuación, entreno mi modelo y voy a ver las predicciones en el conjunto de entrenamiento:
...
El modelo tiene un buen desempeño en el conjunto de entrenamiento, con una especificidad razonable (69.88 %) y accuracy(81.25%).
El modelo en el conjunto de test es mejor que en el modelo de variables escaladas (especificidad del 62.5% y accuracy de 67.35%), lo que sugiere una mejora significativa respecto a versiones previas.

```{r, echo=FALSE, warning=FALSE}
# Eliminamos datos que no usaremos más
...
Otra forma de variar nuestro método es mediante la variación en las variables introducidas.
Variables anteriormente introducidas como athleteid, eventid pueden no estar influyendo.
Además, estas variables son de tipo factor.
Por lo tanto, al no distribuirse de manera lineal, calcular las distancias euclídeas podría no ser la forma correcta para clasificar a nuestros atletas.
...
Como podemos observar, el modelo con mejor valor en accuracy es el k=7.
Tenemos muy cerca también el de k=5.
Por lo que ajustaremos nuestros datos a un modelo knn de k=7 escalado.
...
tabla_knn3<- table(Predicted = predicciones_train_best3, Actual = kn3$target)

# Predicciones para el conjunto de prueba
predicciones_test_best3 <- predict(modelo4knn, newdata = kn3_test)

# Tabla de confusión para el conjunto de prueba
tabla_knn3<- table(Predicted = predicciones_test_best3, Actual = kn3_test$target)
...
Como vemos, la precisión en los datos de entrenamiento estaría en un 68%.
Sin embargo, al ajustar los datos a test, nos resulta en un 46%.
Además, la especificidad es tan sólo de un 25%.
Por tanto, nuestro modelo es bastante deficiente y debemos abandonarlo.
Como observación tenemos que hallando el modelo y sus predicciones para con k=5 volvemos a obtener un 46% de precisión.
Tampoco continuaremos con ese modelo.

```{r, echo=FALSE, warning=FALSE}
#Eliminamos datos que no se usarán en el futuro. 
...
## Modelo 5.(mirar porqué no me funciona.)

En este último modelo, vamos a probar a usar una función que proporciona la librería Caret, la cual va a tratar de darnos la mejor selección de variables.
Para ello, seleccionamos primero todas las variables posibles.
...
Observamos el funcionamiento del modelo con 3 variables:
...
Observamos que tiene un accuracy de 77.4% en Train y 61% en test.
La especificidad es del 41%, significa que el modelo identifica correctamente solo el 41% de las instancias negativas reales como negativas.
Esto implica que un 60% de las instancias negativas reales están siendo clasificadas erróneamente como positivas (falsos positivos).
Por tanto, no es un buen modelo para el objetivo que estamos buscando.

```{r, echo=FALSE}
rm(control, variablesSeleccionadas, df, df_test, matrizconfusion1, matrizconfusion2, predicciones, predicciones_test)
```

## Resumen y decisión. (poner los datos bien.)

A continuación haremos un resumen de los modelos ajustados.

1.  KNN sin escalar y k = 5. Incluye todas las variables, incluidas las categóricas como athleteid y eventidF. En el entrenamiento, Accuracy, 0,797 (79,7%),tasa de falsos positivos ( FPR): 0,434 (43,4%). En prueba, Accuracy, 0,733 (73,3%), tasa de falsos positivos ( FPR): 0,5 (50%).
...
2.  Modelo KNN Escalado.
    Las variables se escalaron para garantizar que todas tengan una contribución equitativa.
    Se utilizó validación cruzada para encontrar el mejor k.
    Mejor valor de k encontrado: 3 .
    En entrenamiento, la precisión ( Accuracy): 0,78.
    Y en prueba, 0,5.

3.  Modelo k=3 sin escalar.
    Entrenamiento,0,7641 (76,41%).
    Esto significa que el modelo clasifica correctamente el 76,41% de los datos en el conjunto de entrenamiento.En prueba 0,6735 (67,35%).
    El modelo clasifica correctamente el 67,35% de los datos en el conjunto de prueba.

4.  Modelo KNN Simplificado (Sin Variables Categóricas).Variables categóricas como athleteidy eventidFse eliminaron.
    Solo se incluyen variables numéricas relevantes.
    Se probaron diferentes valores de k y se encontró que k = 7 es el mejor.En entrenamiento, la precisión ( Accuracy): 0.680 (68%).En prueba, la precisión ( Accuracy): 0,460 (46%).
    Conclusión: El modelo sin variables categóricas y con solo variables numéricas tiene un rendimiento notablemente inferior.
    La eliminación de información clave (como identificadores o categorías relevantes) ha afectado la capacidad del modelo para predecir correctamente.
...
pred_probs_knn_no_scale <- predict(modelo1knn, newdata = df_test[,-14], type = "prob")[, 2]

# 2. KNN escalado
pred_probs_knn_scaled <- predict(modelo2knn, newdata = df_test_scaled[,-14], type = "prob")[, 2]

# 3. KNN sin escalar k=3
pred_probs_knn_es_3 <- predict(modelo3knn, newdata = df_test[,-14], type = "prob")[, 2]

# 4. KNN con variables seleccionadas.
pred_probs_knn_selected <- predict(modelo4knn, newdata = kn3_test, type = "prob")[, 2]

# 5. KNN con las MEJORES variables seleccionadas.
pred_prbos_knn_5 <- predict(modelo4knn, newdata = df_test[,-14], type = "prob")[, 2]
...
KNN sin escalar con k=3, Área bajo la curva: 0,6375.
Es el mejor modelo.
...
A continuación, vamos a configurar varios modelos basados en árboles de decisión.
Los árboles de decisión son un modelo de aprendizaje supervisado ampliamente utilizado por su capacidad de manejar datos categóricos y numéricos, ser interpretables y funcionar bien sin necesidad de escalar.
En este análisis, implementamos un árbol de decisión para predecir la variable objetivo target.
Este tipo de algoritmos determinan qué variables explicativas proporcionan la mayor parte de la información (ganancia de información) para medir la variable objetivo y hacen predicciones probando secuencialmente las características en orden a su información.
Vamos a realizar un árbol de decisión implementando todas las variables que vamos a dar por conocidas PREVIAS a la final.
Es decir, todas las variables menos las que involucren el nado en la final (parciales y tiempo de reacción)
...
A continuación, calculamos el árbol:
...
Según observamos, podemos concluir que nuestro modelo separa según países, los carriles usados en la preliminar y final, y el swimtime de la preliminar, e incluso por el parcial más lento en la preliminar.

Tiene sentido por una parte que use esa variable SwimtimeP, ya que como hemos visto en el análisis exploratorio, contra más larga era la prueba, mayor probabilidad de reducir su tiempo (es decir, target=1) existía.
A continuación, probamos cómo de bueno es nuestro árbol.
Para ello, vemos su matriz de confusión:
...
Nuestro árbol inicial presenta buenos resultados, con una precisión (Accuracy) del 83.1%, lo cual se considera un desempeño notable.
Además, el algoritmo destaca por su alta sensibilidad (Sensitivity) de 91.1%, evidenciando una gran capacidad para clasificar a los nadadores que mejoran sus tiempos, cometiendo errores en aproximadamente 1 de cada 10 casos.
Por otro lado, la especificidad del modelo es del 72%, lo que indica que identifica correctamente al 72% de los nadadores que no mejoran sus tiempos.

Probamos el árbol en el conjunto test.
...
Se observa una disminución significativa en su desempeño, lo que podría indicar la presencia de sobreajuste en el modelo.
Se logra una sensibilidad del 76%, lo que significa que clasifica correctamente a ese porcentaje de los nadadores que mejoran sus tiempos.
Sin embargo, la especificidad es de 45.83%, lo que indica que solo identifica correctamente a ese porcentaje de los nadadores que no mejoran sus tiempos, resultando en una alta tasa de falsos positivos.
Esto refuerza la necesidad de reevaluar los parámetros y técnicas utilizadas para evitar el sobreajuste.

```{r, warning=FALSE}
rm(cf_test, cf_train, prediction)
```
...
Para ello, emplearemos la función rpart.control, que permite definir parámetros ajustados según los requerimientos, como el número de nodos terminales y la profundidad máxima del árbol.
Esta configuración asegurará un mayor control sobre la estructura y complejidad del modelo.
...
Con estos parámetros establecidos, volvemos a ejecutar la función rpart y ver el árbol resultante.
...
# Visualizar el árbol
#rpart.plot(arbol2, type = 3, extra = 104, under = TRUE, cex = 0.8)
```
...
A continuación, voy a proceder a hacer las predicciones correspondientes en nuestros conjuntos de datos tanto de train como de test.
...
Nuestro modelo, en el conjunto de datos de entrenamiento, es bastante bueno, ya que acierta casi todos las predicciones 1, y además tiene un accuracy del casi 90%.
...
La sensibilidad (94.64%) es excelente, lo que significa que el modelo es muy eficaz para identificar correctamente los casos positivos (en este caso, los nadadores que bajan de tiempo).
La especificidad (78.31%) también es razonable, indicando que el modelo es relativamente bueno para identificar los casos negativos, aunque podría mejorarse.
...
En este caso, tiene un accuracy de 57.14%, lo que indica que el modelo acierta en aproximadamente la mitad de las predicciones y el valor de especificidad es relativamente bajo, lo que sugiere que el modelo está cometiendo bastantes falsos positivos.

```{r, warning=FALSE}
rm(cf_test, cf_train, control_tree, pred_test, pred_train)
```

## Modelo 3.

Supongamos que, a partir de ahora, vamos a tener también la variable reactionTimeF, es decir, vamos a predecir si bajan o no después de conocer su salida.
Para ello, volvemos a crear nuestros dataframes y hacemos el proceso análogo a los anteriores modelos:

```{r}
df_arbol <- datos2015Target_train %>% select(target, reactiontimeF, distance, stroke, gender, name,heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)
df_test_arbol <- datos2015Target_test %>% select(target,reactiontimeF, distance, stroke, gender, name,heatF, laneF, edad, heatP, laneP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, medianaParcialesP, maximoParcialesP, minimoParcialesP)
...
```{r}
arbol3 <- rpart(target~., data = df_arbol, method = 'class')
rpart.plot(arbol1, extra = 1)
```

Observamos que el árbol obtenido es el mismo que en el caso anterior.
El tiempo de reacción no parece ser influyente en la reducción del tiempo de los nadadores.
Al menos, a nivel general.

Para corroborar que los resultados van a ser equivalentes, objetivo meramente lúdico, evaluamos el árbol en train y test.
En primer lugar,

```{r}
prediction <- predict(arbol3, df_arbol, type = 'class')
cf_train <- confusionMatrix(prediction, as.factor(df_arbol$target),positive="1",
                      mode = 'everything')
...
```{r}
prediction <- predict(arbol3, df_test_arbol, type = 'class')
cf_test <- confusionMatrix(prediction, as.factor(df_test_arbol$target),positive="1",
                      mode = 'everything')
...
Obtenemos el mismo modelo que antes, como habíamos explicado previamente.

```{r}
rm(cf_train, cf_test, prediction)
```
...
```{r}
# Árbol reducido eliminando variables con baja importancia
arbol4 <- rpart(target ~ ., 
                        data = df_arbol %>% select(maximoParcialesP, swimtimeP, medianaParcialesP,target), 
                        method = "class", 
...
En concreto, es relevante si los nadadores tienen un tiempo mayor o menor de 106 segundos, probablemente indique una división entre las pruebas de larga y baja distancia.
Posteriormene divide según las medianas de dichos parciales.
...
El accuracy de 61.22% indica que el modelo acierta en aproximadamente el 61% de las predicciones.
Aunque es superior a una predicción aleatoria, el p-valor de 0.09887 sugiere que esta diferencia no es estadísticamente significativa.
Esto sugiere que el modelo no está significativamente mejor que una estrategia aleatoria y podría necesitar ajustes o mejoras.

La sensibilidad de 72% indica que el modelo tiene una buena capacidad para identificar los casos positivos (por ejemplo, nadadores que "bajan").
Acierta en 7 de cada 10 casos positivos, lo cual es positivo, aunque por sí solo este estadístico no indica un buen modelo ya que se necesita un equilibrio con la especificidad.
La especificidad de 50% muestra que el modelo identifica correctamente la mitad de los casos negativos.
Sin embargo, la otra mitad de los casos negativos son clasificados erróneamente como positivos, lo que significa que el modelo está generando una cantidad significativa de falsos positivos.

```{r}
rm(cf_train, cf_test, prediction,arbol_cv)
```
...
A continuación, vamos a construir un modelo de bagging sobre el modelo 3 hecho en Árboles de decisión, buscamos de alguna manera generar una mejora.
Para ello, primeramente voy a crear mis dataFrames:
...
Una vez definidos mis árboles, voy a entrenar mi modelo:
...
Vemos sus predicciones en entrenamiento:
...
Parece que sobreajusta ya que tenemos un accuracy, sensitivity y specificity del 100%.
Probamos este modelo con los datos en test:
...
Confirmamos que tiende al sobreajuste, se reducen todos los estadísticos.
En concreto, la especificidad baja a un 45.83%.
Un valor de 45.83% es relativamente bajo, lo que significa que el modelo está cometiendo muchos falsos positivos, clasificando incorrectamente como positivos a muchos casos negativos.

En el siguiente modelo, vamos a tratar de definir mejor el control sobre el árbol creado.
...
Una vez definidos los árboles, entrenamos el modelo:
...
Obtenemos igual que en el caso anterior, unos estadísticos del 100%.
...
Confirmamos que tiende al sobreajuste, aunque se comporta ligeramente mejor que en el caso anterior.
en el siguiente modelo, vamos a tratar de definir mejor el control sobre el árbol creado.
...
### Modelo 3. (preguntar a Carmen)

(hacer este modelo con el MEJOR MODELO de k-vecinos. (creo que se puede))

# Random Forest (Salma e INés)

(hacer un modelo sobre el mejor modelo de árboles de decisión.)
...
Aqui hay algo abajo que esta mal, el que?
no lo se
