...
Esta nueva tabla no repite a los nadadores. Pero debemos tener en cuenta para futuras observaciones, que sólo nos da los resultados de la ronda preliminar en el primer split, ya que es la primera observación que se toma de cada participante.

Dicho esto, si queremos para posibles análisis de rendimiento también el desempeño de cada nadador en su ronda final, haremos otra partición de los datos.
```{r}
nadadoresRondaFinal <- datos2015%>%
  filter(round == "FIN") %>%
  distinct(athleteid, .keep_all = TRUE)
```
```{r}
nadadoresRondaFinal
```
De esta forma, esta partición de los datos nos muestra a los nadadores no repetidos, con los eventos observados en su ronda final en el primer split.

Procedemos ahora a analizar los datos conforme con la edad de los participantes. Para ello, utilizamos la primera partición. Es decir, la de los nadadores en su primer split en la ronda preliminar. 
...
```
Con este gráfico, podemos ver como la gran parte de los participantes compiten a primera hora de la mañana. Nos podríamos preguntar si según el tipo de carrera los horarios se establecen a distintas horas del día, o si el buen o mal desempeño puede ser a causa de estos horarios. 
