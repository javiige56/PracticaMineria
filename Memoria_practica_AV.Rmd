...
Veamos la primera ocurrencia de nuestra tabla:
...
Utilizaremos esta tabla para realizar análisis sobre el número de nadadores, proporción entre hombres y mujeres, la edad de los participantes, etc.
...
rm(componentess, Cor_CompVar, Cor_CompVar_retenidos, Cor_CompVar_retenidos_mayores, prueba200MariposaMasc, prueba200MariposaMasc2, R, resumen200, auto)
```
...
Voy a crear a continuación una tabla en la cual, tenga el nombre de las nadadoras del 800 libres femenino. Además, quiero los parciales al paso por cada 50 y el tiempo final. 
...
Empecemos creando las tablas de manera análoga a como lo hicimos en el PCA: 

```{r, warning=FALSE}
prueba800libresPreliminar<- datos2015[datos2015$distance==800 & datos2015$gender=="F" & datos2015$stroke=="FREE" & datos2015$round=="PRE", ]

prueba800libresPreliminar <- prueba800libresPreliminar %>%
    dplyr::select(lastname,swimtime, splitdistance, splitswimtime)
...
A continuación, vamos a echarle un vistazo a la tabla: 
...
```{r}
distance <- get_dist(free800WomensNormalizado[,-1])
...
```{r}
cluster800libres <- kmeans(free800WomensNormalizado[,-1], centers = 3, nstart = 25)
```

```{r}
set.seed(123)  # Para reproducibilidad
kmeans_model <- kmeans(datos_normalizados[, c("50", "100", "150", "200", "250", 
                                              "300", "350", "400", "450", "500", 
                                              "550", "600", "650", "700", "750", 
                                              "800")], centers = 3)

# Añade los grupos a los datos
datos_normalizados$grupo <- as.factor(kmeans_model$cluster)

# Transformar los datos a formato largo para ggplot
datos_largos <- datos_normalizados %>%
  pivot_longer(cols = c("50", "100", "150", "200", "250", "300", "350", "400", 
                        "450", "500", "550", "600", "650", "700", "750", "800"), 
               names_to = "parcial", values_to = "tiempo_normalizado")

# Graficar
ggplot(datos_largos, aes(x = as.numeric(parcial), y = tiempo_normalizado, 
                         group = nombre, color = grupo)) +
  geom_line(alpha = 0.6) +
  labs(x = "Parcial (m)", y = "Tiempo Normalizado", color = "Grupo") +
  theme_minimal() +
  ggtitle("Estrategias de Carrera Agrupadas por Clustering")
```
