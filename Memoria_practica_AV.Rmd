...
```
La variable 200, por ejemplo, está explicada fundamentalmente por la dimensión 1 (componente 1), lo mismo que le ocurre a la variable 150, en menor medida. La primera, representa un color amarillo medio, lo que indica un porcentaje de varianza explicada cercano al 37%. No debería llamarnos la atención es ver que la edad esta explicada con la componente 1, en un color muy oscuro,entorno al 74%
```{r}
...
En este caso, se representa la suma de cos2 para los 2 componentes.La proporción de variabilidad explicada por los tres componentes retenidos bastante baja.
...
Cuanto más cercana esté una variable al borde, mejor será la calidad de la representación en el conjunto de las dos componentes. Las variables que están cercanas al centro del círculo no están suficientemente bien representadas por esos componentes, como ocurre con reactiontime(ya lo teníamos comprobado anteriormente), pero el resto de variables están bien representadas por estas dos dimensiones.
...
Esto es una manera interesante de ver que escoger dos componentes sería lo adecuado:
