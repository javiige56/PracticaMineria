...
- Se implementaron distancias sobre las variables relevantes continuas para el análisis. 
...
Por lo tanto, podemos concluir con que el modelo falla demasiado en nuestros datos de entrenamiento. Luego, no va a ser un buen modelo. Aún así, será nuestro modelo base para intentar encontrar uno mejor. 
...
El modelo muestra un buen ajuste en los datos de entrenamiento, con alta sensibilidad y precisión. Sin embargo, existe un leve riesgo de sobreajuste, ya que el desempeño en el entrenamiento es relativamente alto. Esto deberá confirmarse evaluando el modelo en el conjunto de prueba:
...
El modelo muestra una pérdida notable de rendimiento al pasar de entrenamiento a prueba. Esto podría deberse a sobreajuste, donde el modelo aprende demasiado de los patrones específicos del conjunto de entrenamiento y no generaliza bien en datos no vistos.

58.83%, que es el promedio de la sensibilidad y la especificidad. Muestra que el modelo no está logrando un equilibrio óptimo entre las clases positivas y negativas.
...
(hacer algo gráfico que muestre que arbol es mejor)
