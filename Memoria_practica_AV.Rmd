...
-   Las pruebas nadadas en 50m y 100m son los 4 estilos.
...
## Estudio sobre la relación entre la edad de los nadadores y las distancias que nadan. (REVISAR)
...
```{r, echo=FALSE}
rm(participaciones_menores,resumen_pruebas, total_nadadores, nadadorasPruebas, nadadoresPruebasM)
...
### Knn suponiendo que tenemos el reactionTime de la final. 
...
Primeramente, vamos a probar metiendo variables que razonablemente podrían determinar los mejores vecinos. Para ello, meto: 
- Athelteid: si una nadadora ha bajado en todas sus pruebas anteriores, pinta que puede ser determinante.
- eventidF: es el id de evento de la final. Si meto esta variable, ya no me hace falta ni el sexo, ni el estilo ni la distancia porque dependen de esta variable. 
- reactionTimeF: Supondremos que vamos a predecir si los nadadores bajan justo en el momento en el que saltan. 
- edad: También puede ser importante. 
- eventidP: al igual que el de la final, nos va a "acercar" nadadoras que naden la misma prueba. 
...
# Primeramente pruebo sin escalar. A ver qué sale: 

prob1 = knn3(df[,-14], as.factor(df$target), k=5, prob=TRUE)


# Obtener las predicciones de clase
predicciones <- predict(prob1, newdata = df[,-14], type = "class")
table(Predicted = predicciones, Actual = df$target)
...
predicciones_test <- predict(prob1, newdata = df_test[,-14], type = "class")
table(Predicted=predicciones_test, Actual=df_test$target)



```

Evaluamos el rendimiento de esta tabla anterior y observamos que: 
...
No parece un modelo bueno en el propio conjunto de entrenamiento. En el conjunto de test: 
...
Es como lanzar una moneda al aire. No es un buen modelo. Vamos a probar a escalar, meter otras variables, e incluso probar con otras matrices de distancia para intentar mejorarlo. También, Vamos a probar a bajar el número proximo de vecinos. Podemos buscar cómo ver cómo se ha ido construyendo las predicciones. Es decir, los vecinos los cuales eran 0 o 1 y a partir de ahí como se ha construido su target.



Dado que KNN depende de la distancia entre las observaciones, las variables con rangos más amplios podrían influir desproporcionadamente en los resultados. Por lo tanto, es fundamental realizar un escalado antes de aplicar KNN para garantizar que todas las variables contribuyan equitativamente.
...
```
Guarda el valor de k que obtuvo el mejor rendimiento durante la validación cruzada(3).

```{r}
set.seed(1466)
#Entrenar el modelo KNN con el mejor k encontrado, que es 3
knn_best <- knn3(df_scaled[,-14], as.factor(df_scaled$target), k = 3, prob = TRUE)

# Predicciones para el conjunto de entrenamiento
predicciones_train_best <- predict(knn_best, newdata = df_scaled[,-14], type = "class")
...
predicciones_test_best <- predict(knn_best, newdata = df_test_scaled[,-14], type = "class")
...
```
El modelo tiene un buen rendimiento en el conjunto de entrenamiento con una precisión alta (79.7%) y un equilibrio razonable entre sensibilidad y especificidad. Esto indica que el modelo está capturando patrones en los datos de entrenamiento.

Luego,tiene mejor precisión general en el conjunto de entrenamiento (79.7% frente a 72.5% antes). Clasifica mejor la clase 1 tanto en entrenamiento como en prueba.

Debido a una sospecha pruebo el siguiente modelo sin escalar pero con **k=3**.

```{r}
set.seed(125)
#Entrenar el modelo KNN con el mejor k encontrado
knn_best2 <- knn3(df[,-14], as.factor(df$target), k = 3, prob = TRUE)

# Predicciones para el conjunto de entrenamiento
predicciones_train_best2 <- predict(knn_best2, newdata = df[,-14], type = "class")

# Tabla de confusión
table(Predicted = predicciones_train_best2, Actual = df$target)


# Predicciones para el conjunto de prueba escalado
predicciones_test_best2 <- predict(knn_best2, newdata = df_test[,-14], type = "class")

# Tabla de confusión
table(Predicted = predicciones_test_best2, Actual = df_test$target)
...
#el atletheid no nos intereresa
df_arbol <- df %>% select(-athleteid)
df_test_arbol <- df_test %>% select(-athleteid)
...
```
La variable swimtimePes el predictor más importante.Si swimtimeP < 106, se clasifica en ramas basadas en otros factores como medianaParcialesP y pointP. Si swimtimeP >= 106, se usan variables adicionales como medianaParcialesP para la clasificación.
...
confusionMatrix(pred_test, as.factor(df_test$target))
...
Puedes usar validación cruzada para encontrar el mejor valor de cp que maximice la generalización del árbol.
...
confusionMatrix(pred_test_reducido, as.factor(df_test_arbol_reducido$target))
