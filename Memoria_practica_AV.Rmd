...
- Ajustar parámetros como ntree, mtry y nodesize.
  ...
  * mtry: número de variables aleatorias que se consideran para cada división en un árbol. Un valor demasiado bajo puede subajustar, mientras que un valor demasiado alto puede hacer que los árboles se sobreajusten. Usualmente se toma el valor de la raiz cuadrada del número de predictores.
...
# 3. Definir los parámetros a optimizar
# Búsqueda en cuadrícula para el parámetro mtry (número de predictores por división)
tune_grid <- expand.grid(.mtry = c(3, 5, 7, 9))  # Puedes ajustar estos valores

# 4. Ajustar el modelo Random Forest usando validación cruzada y búsqueda de parámetros
rf_model <- randomForest(
  target ~ .,                 # Fórmula con la variable dependiente y todos los predictores
  data = df_random_std,       # Datos estandarizados
  method = "rf",              # Usar Random Forest
  trControl = train_control, 
  tuneGrid = tune_grid,  # Validación cruzada       # Búsqueda en cuadrícula para mtry
...
clase_pred_rf_train <- ifelse(pred_rf_train > 0.5, "1", "0")  
cf_rf_train <- confusionMatrix(as.factor(clase_pred_rf_train), as.factor(df_random$target))
...
cf_rf_test <- confusionMatrix(as.factor(clase_pred_rf_test), as.factor(df_test_random$target))
print(cf_rf_test)

```

(no se por que no me salen los resultados pero bueno ahora lo vuelvo a inicializar todo)
