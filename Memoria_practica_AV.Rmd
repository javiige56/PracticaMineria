...
En los modelos que vamos a calcular a continuación, nuestro objetivo va a ser que, cada vez que digamos que un nadador baja de tiempo, sea cierto, es decir, quiero minimizar la tasa de falsos positivos. 
...
En este caso, tiene un accuracy de 57.14%, lo que indica que el modelo acierta en aproximadamente la mitad de las predicciones. Además, detecta 1 de cada 2 positivos aproximadamente, una moneda al aire. No es un buen modelo. 
...
Observamos que no se equivoca nunca, es decir, reconoce todos los patrones en el conjunto en el que ha entrenado. Esto, sugiere sobreajuste tal y como hemos aprendido durante el proyecto. Visualicemos a continuación los datos en test: 
...
Tal y como preveíamos, se han sobreajustado los datos y este modelo entrenado no es demasiado válido, es como lanzar una moneda al aire. 
...
```
A nivel global, podemos ver que las características mas influyentes han sido: la calle en preliminar y finales, el tiempo en los parciales y el tipo de nado.

(COMENTARIO: ESTO TIENE ALGUN TIPO DE SENTIDO??¿¿)
...
df_test<- datos2015Target_test%>% select(target, swimtimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)
scaled_df_test <- data.frame(
  target = df$target,  
  scale(df[, -1])      
)

# Para devolver clases:
#predict_knn <- predict(modelo3knn, newdata = scaled_df_test)


# Crear un predictor para el modelo

# Crear un objeto Predictor para el modelo
#predictor_knn <- Predictor$new(
#  model = modelo3knn,
#  data = scaled_df[,-1],   # Solo características
#  y = scaled_df$target,   # Variable objetivo
  #predict.function = predict_knn  # Función personalizada
#)

```

Posteriormente, se calculan los valores SHAP para la primera fila.
```{r}
# Calcular valores SHAP para una observación específica (primera fila)
#shap_knn <- Shapley$new(
#  predictor = predictor_knn,
#  x.interest = scaled_df[1, ] 
)
