...
rm(conf_matrix_test, conf_matrix_train, control, matrizconfusion1, matrizconfusion2, knnFit, preProcValues, tuneGrid)
```
...
En este último modelo, vamos a tratar de elegir las mejores variables que consiguen predecir el Target. Para ello, vamos a hacer uso del análisis previo, y vamos a tratar de ponderar cada una por un peso para tratar de predecir mejor. Primeramente, vamos a elegir qué variables meto: 

```{r}
df <- datos2015Target_train %>% select(target, distance, eventidF, reactiontimeF, edad, eventidP, pointP, reactiontimeP, swimtimeP, daytimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)

df_test <-  datos2015Target_test %>% select(target, distance, eventidF, reactiontimeF, edad, eventidP, pointP, reactiontimeP, swimtimeP,daytimeP, mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP)

```

Bien, he elegido estas variables. Ahora voy a escalarlas: 

```{r}
# Nombre de la columna que no quieres escalar
columna_no_escalar <- "target"
df <- df %>%
  mutate(across(-all_of(columna_no_escalar), ~ scale(.) %>% as.vector()))
...
A continuación, voy a añadir mayor importancia a las variables: 
- swimtimeP
- medianaParcialesP
- edad
- pointsP
...
df <- df %>%
  mutate(
    #swimtimeP = swimtimeP * 3,
    #edad = edad * 2,
    #medianaParcialesP= medianaParcialesP*2.5,
    #pointP=pointP*2
  )

df_test <- df_test %>%
  mutate(
    #swimtimeP = swimtimeP * 2,
    #edad = edad * 2,
    #medianaParcialesP= medianaParcialesP*2.5,
    #pointP=pointP*2
  )
