...
El modelo tiene un buen rendimiento en el conjunto de entrenamiento con una precisión y sensibilidad alta (78% y 85.71%, respectivamente). Esto indica que el modelo está capturando patrones en los datos de entrenamiento. Sin embargo, en el test ambas caen, 53.06% (accuracy), 72% (sensitivity), lo que sugiere un sobreajuste a mis datos de entrenamiento.
...
El modelo tiene un buen desempeño en el conjunto de entrenamiento, con una sensibilidad razonable (76.41 %) y accuracy(81.25%). El modelo en el conjunto de test es mejor que en el modelo de variables escaladas (sensitivity del 72%  y accuracy de 67.35%), lo que sugiere una mejora significativa respecto a versiones previas.
...
Como vemos, la precisión en los datos de entrenamiento estaría en un 68%. Sin embargo, al ajustar los datos a test, nos resulta en un 46%. Por tanto, nuestro modelo es bastante deficiente. 
Como observación tenemos que hallando el modelo y sus predicciones para con k=5 volvemos a obtener un 46%. Debemos abandonar este modelo.
...
(HAY QUE MIRAR SPECIFITY EN VEZ DE SENSITIVITY)
...
Tiene sentido por una parte que use esa variable SwimtimeP, ya que como hemos visto en el análisis exploratorio, contra más larga era la prueba, mayor probabilidad de bajar (es decir, target=1) existía. Bien, a continuación, vamos a testear cómo de bueno es nuestro árbol. Para ello, veamos su matriz de confusión: 
...
Tiene buena pinta nuestro primer modelo, si observamos, tenemos un Accuracy del 0.831, lo cuál es bastante bueno. Además, es un algoritmo con muy buena sensitividad, es decir, clasifica bien los nadadores que bajan, se equivoca en ello 1 de cada 10 veces (0.911). 



Ahora lo pruebo en mi conjunto de test: 
```{r}
...
Para nuestro conjunto test, el modelo baja bastante, luego puede indicar que existe algún tipo de sobreajuste en el modelo anterior. Sí que acierta 3 de cada 4 nadadores cuando predice que baja, pero el accuracy es prácticamente tirar una moneda al aire. 
...
La función que crea el arbol para el modelo anterior, tiene como valores defecto algunos valores que no voy a querer usar o que voy a querer ajustar de manera personalizada. Por ello es que, con la función rpart.control, voy a definir mis propios parámetros sobre los nodos finales y longitud del árbol: 
...
Bien, con estos parámetros establecidos, vuelvo a ejecutar la función rpart y a ver el árbol resultante: 
...
(Salma mira esta gráfica para mejorarla un poco.)

Cada hoja indica una predicción (0 o 1), con precisión y porcentaje de datos representados.
A continuación, voy a proceder a hacer las predicciones correspondientes en mis conjuntos de datos tanto de train y test: 
...
Nuestro modelo, en el conjunto de datos de entrenamiento, es bastante bueno, ya que acierta casi todos las predicciones 1, y además tiene un accuracy del casi 90%. Veamos ahora en los datos test:
...
Obtenemos el mismo modelo que antes, luego, en nuestros modelos no es importante dicha variable.
