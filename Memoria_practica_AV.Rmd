...
Hay 185 países. Debido a la cantidad de países, ver por países cuántos nadadores hay no va a ser muy interpretable.
...
A partir de los 200 metros, se observa una tendencia en la que los nadadores parecen reservar parte de su energía durante las etapas iniciales para maximizar su rendimiento en las finales, donde efectivamente logran disminuir sus tiempos. Este comportamiento sugiere que es necesario considerar este factor en el análisis.
...
El método de los k vecinos más cercanos (abreviatura de “k nearest neighbors” en inglés), se cuenta entre los enfoques más simples y ampliamente utilizados en el campo del Machine Learning. Este método se basa en la noción de similitud (o distancia) entre observaciones.
...
- reactionTimeF. Supondremos que vamos a predecir si los nadadores bajan justo en el momento en el que saltan. Esta variable es relevante para predecir si un nadador bajará su tiempo. Si su tiempo de reacción en la final es excesivamente alto, podría influir en la decisión de nuestro modelo.

- edad. Puede ser importante, ya que la edad puede influir en el rendimiento, tal y como hemos observado previamente.

- eventidP. Similar al EventidF, el evento preliminar puede agrupar a los nadadores que compiten en las mismas condiciones.  

- pointP. Puntos de las preliminares. 

- reactionTimeP. Tiempo de reacción de la prueba previa. 

- swimtimeP. Tiempo final de la prueba previa.

- daytimeP. Hora de la preliminar.

- mediaParcialesP, minimoParcialesP, maximoParcialesP, medianaParcialesP. Son bastante útiles, ya que proporcionan información más detallada sobre el rendimiento del nadador a lo largo de la prueba. El promedio de los parciales puede indicar la consistencia del nadador. El máximo, el mínimo, la media y la mediana pueden ayudar a identificar vecinos con características parecidas sobre cómo plantean la prueba.
...
La precisión del modelo está en un 57%. Es decir, acierta prácticamente por azar. Por tanto,no es un buen modelo. Probaremos con algunas modificaciones esperando conseguir un modelo mejor: escalar, introducir o descartar variables y/o elegir un k distinto.
...
Dado que KNN depende de la distancia entre las observaciones, las variables con rangos más amplios podrían influir desproporcionadamente en los resultados. Por lo tanto, escalaremos antes de aplicar KNN. De esta forma, todas las variables contribuirán equitativamente.
...
Con el objetivo de obtener un modelo más preciso, utilizaremos la validación cruzada para encontrar el mejor k. Es decir, el número ideal de observaciones más cercanas (vecinos) en las que basar nuestras predicciones.
...
Tenemos en los resultados que el mejor k es 3. Por tanto, evaluamos el modelo con k=3 y las variables escaladas. Posteriomente, entrenamos el modelo y analizamos la matriz de confusión en los datos de entrenamiento.
...
Parece que se produce una mejora respecto al anterior modelo tanto en accuracy como en sensitivity. A continuación, evaluamos el modelo en el conjunto de test. Para ello, escalamos los datos test.
...
El modelo tiene un buen rendimiento en el conjunto de entrenamiento con una precisión y especificidad alta (78.97% y 69.88%, respectivamente). Esto indica que el modelo está capturando patrones en los datos de entrenamiento. Sin embargo, en el test ambas caen, 53.06% (accuracy), 33.33% (especificidad), lo que sugiere un sobreajuste a mis datos de entrenamiento.
...
A continuación, probamos a usar k=3 con los datos sin escalar. Comenzamos nuevamente seleccionando las variables.
...
El modelo tiene un buen desempeño en el conjunto de entrenamiento, con una especificidad razonable (69.88 %) y accuracy(81.25%). El modelo en el conjunto de test es mejor que en el modelo de variables escaladas (especificidad del 62.5%  y accuracy de 67.35%), lo que sugiere una mejora significativa respecto a versiones previas.
...
Como podemos observar, el modelo con mejor valor en accuracy es el k=7. Tenemos muy cerca también el de k=5. Por lo que ajustaremos nuestros datos a un modelo knn de k=7 escalado.
...
Como vemos, la precisión en los datos de entrenamiento estaría en un 68%. Sin embargo, al ajustar los datos a test, nos resulta en un 46%. Además, la especificidad es tan sólo de un 25%. Por tanto, nuestro modelo es bastante deficiente y debemos abandonarlo. 
Como observación tenemos que hallando el modelo y sus predicciones para con k=5 volvemos a obtener un 46% de precisión. Tampoco continuaremos con ese modelo.
...
En este último modelo, vamos a probar a usar una función que proporciona la librería Caret, la cual va a tratar de darnos la mejor selección de variables. Para ello, seleccionamos primero todas las variables posibles. 
```{r}
library(caret)
```
...
Observamos que tiene un accuracy de 77.4% en Train y 61% en test. La especificidad es del 41%, significa que el modelo identifica correctamente solo el 41% de las instancias negativas reales como negativas. Esto implica que un 60% de las instancias negativas reales están siendo clasificadas erróneamente como positivas (falsos positivos). Por tanto, no es un buen modelo para el objetivo que estamos buscando.
...
A continuación, vamos a configurar varios modelos basados en árboles de decisión. Los árboles de decisión son un modelo de aprendizaje supervisado ampliamente utilizado por su capacidad de manejar datos categóricos y numéricos, ser interpretables y funcionar bien sin necesidad de escalar. En este análisis, implementamos un árbol de decisión para predecir la variable objetivo target. Este tipo de algoritmos determinan qué variables explicativas proporcionan la mayor parte de la información (ganancia de información) para medir la variable objetivo y hacen predicciones probando secuencialmente las características en orden a su información. Vamos a realizar un árbol de decisión implementando todas las variables que vamos a dar por conocidas PREVIAS a la final. Es decir, todas las variables menos las que involucren el nado en la final (parciales y tiempo de reacción)
...
Tiene sentido por una parte que use esa variable SwimtimeP, ya que como hemos visto en el análisis exploratorio, contra más larga era la prueba, mayor probabilidad de reducir su tiempo (es decir, target=1) existía. A continuación, probamos cómo de bueno es nuestro árbol. Para ello, vemos su matriz de confusión: 
...
Nuestro árbol inicial presenta buenos resultados, con una precisión (Accuracy) del 83.1%, lo cual se considera un desempeño notable. Además, el algoritmo destaca por su alta sensibilidad (Sensitivity) de 91.1%, evidenciando una gran capacidad para clasificar a los nadadores que mejoran sus tiempos, cometiendo errores en aproximadamente 1 de cada 10 casos.
...
Se observa una disminución significativa en su desempeño, lo que podría indicar la presencia de sobreajuste en el modelo. Se logra una sensibilidad del 76%, lo que significa que clasifica correctamente a ese porcentaje de los nadadores que mejoran sus tiempos. Sin embargo, la especificidad es de 45.83%, lo que indica que solo identifica correctamente a ese porcentaje de los nadadores que no mejoran sus tiempos, resultando en una alta tasa de falsos positivos.
...
La función utilizada para generar el árbol en el modelo anterior incluye valores predeterminados que no se ajustan completamente a las necesidades de este análisis, por lo que será necesario personalizarlos. Para ello, emplearemos la función rpart.control, que permite definir parámetros ajustados según los requerimientos, como el número de nodos terminales y la profundidad máxima del árbol. Esta configuración asegurará un mayor control sobre la estructura y complejidad del modelo.
...
En general, el modelo muestra un buen rendimiento: la precisión (87.69%) es bastante alta, lo que indica que la mayoría de las predicciones son correctas. La sensibilidad (94.64%) es excelente, lo que significa que el modelo es muy eficaz para identificar correctamente los casos positivos (en este caso, los nadadores que bajan de tiempo). La especificidad (78.31%) también es razonable, indicando que el modelo es relativamente bueno para identificar los casos negativos, aunque podría mejorarse.
...
cf_test
```
El modelo tiene un buen desempeño en el conjunto de entrenamiento, pero tiene un bajo desempeño en datos no vistos, lo que sugiere sobreajuste. En este caso, tiene un accuracy de 57.14%, lo que indica que el modelo acierta en aproximadamente la mitad de las predicciones y el valor de especificidad es relativamente bajo, lo que sugiere que el modelo está cometiendo bastantes falsos positivos.
...
Supongamos que, a partir de ahora, vamos a tener también la variable reactionTimeF, es decir, vamos a predecir si bajan o no después de conocer su salida. Para ello, volvemos a crear nuestros dataframes y hacemos el proceso análogo a los anteriores modelos: 
...
rpart.plot(arbol1, extra = 1)
```
Observamos que el árbol obtenido es el mismo que en el caso anterior. El tiempo de reacción no parece ser influyente en la reducción del tiempo de los nadadores. Al menos, a nivel general.

Para corroborar que los resultados van a ser equivalentes, objetivo meramente lúdico, evaluamos el árbol en train y test. En primer lugar, 
...
```
En este árbol toma la variable más importante el tiempo de nado en el parcial. En concreto, es relevante si los nadadores tienen un tiempo mayor o menor de 106 segundos, probablemente indique una división entre las pruebas de larga y baja distancia. Posteriormene divide según las medianas de dichos parciales. Por último, utiliza nuevamente los tiempos de los parciales. 
...
cf_test
```
El accuracy de 61.22% indica que el modelo acierta en aproximadamente el 61% de las predicciones. Aunque es superior a una predicción aleatoria, el p-valor de 0.09887 sugiere que esta diferencia no es estadísticamente significativa. Esto sugiere que el modelo no está significativamente mejor que una estrategia aleatoria y podría necesitar ajustes o mejoras.

La sensibilidad de 72% indica que el modelo tiene una buena capacidad para identificar los casos positivos (por ejemplo, nadadores que "bajan"). Acierta en 7 de cada 10 casos positivos, lo cual es positivo, aunque por sí solo este estadístico no indica un buen modelo ya que se necesita un equilibrio con la especificidad.
La especificidad de 50% muestra que el modelo identifica correctamente la mitad de los casos negativos. Sin embargo, la otra mitad de los casos negativos son clasificados erróneamente como positivos, lo que significa que el modelo está generando una cantidad significativa de falsos positivos.
...
Los métodos de ensamblado son técnicas avanzadas de aprendizaje automático que combinan múltiples modelos base para mejorar la precisión, la robustez y la generalización de las predicciones. El objetivo principal de estos métodos es reducir el error del modelo base mediante la integración de varios predictores, lo que permite capturar diferentes patrones en los datos. 

## Bagging.
Bagging es un método de ensamblado paralelo que busca reducir la varianza de los modelos base, mejorando la precisión general de las predicciones. Este enfoque es particularmente útil en algoritmos inestables, como árboles de decisión, que pueden producir modelos muy diferentes con ligeros cambios en los datos de entrenamiento.

### Modelo 1.
A continuación, vamos a construir un modelo de bagging sobre el modelo 3 hecho en Árboles de decisión, buscamos de alguna manera generar una mejora. Para ello, primeramente voy a crear mis dataFrames: 
...
Confirmamos que tiende al sobreajuste, se reducen todos los estadísticos. En concreto, la especificidad baja a un 45.83%. Un valor de 45.83% es relativamente bajo, lo que significa que el modelo está cometiendo muchos falsos positivos, clasificando incorrectamente como positivos a muchos casos negativos. 
